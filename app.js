...
function escapeJson(o){return escapeHtml(JSON.stringify(o))}
...
function initPractice(){
  ...
  $("#practiceList").innerHTML=pick.map(x=>`<div class="task" style="margin-bottom:10px">
    <div class="row" style="justify-content:space-between"><div class="q">${escapeHtml(x.sectionTitle)} – Aufgabe ${x.taskIndex+1}</div><span class="badge">${escapeHtml(x.task.type)}</span></div>
    <div style="margin-top:10px">${renderTask(x.topicId,x.sectionId,x.task,x.taskIndex).replace(/^[\s\S]*?<div class="task"[^>]*>/,"").replace(/<\/div>\s*$/,"")}</div></div>`).join("");
  attachInteractiveHandlers();
}
...
function initQuiz(isUnit){
  let pool=[];
  if(isUnit){
    DATA.topics.forEach(t=>(t.sections||[]).forEach(s=>(s.tasks||[]).forEach((task,i)=>{if(["mcq","truefalse","fill"].includes(task.type))pool.push({topicTitle:t.title,sectionTitle:s.title,task})}))));
